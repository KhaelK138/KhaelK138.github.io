...
If your game supports user-generated content, treat it as untrusted code--allowlist asset file types in Workshop uploads and restrict UE5 Blueprint usage. This vulnerability has been patched in the latest versions, with the game no longer mounting arbitrary files to disk and only accepting files with whitelisted extensions. I've also reached out to Epic Games to hopefully get some secure defaults for `LaunchURL` in UE5, where only safe HTTP/HTTPS links can be launched by default.  