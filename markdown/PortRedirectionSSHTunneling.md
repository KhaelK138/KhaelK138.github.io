...
- Then after starting server and connecting like normal:
  - Select the session within Ligolo and run `start`, which will install the driver and set up the interface
  - Show interfaces with: `netsh int ipv4 show interfaces`
    - Get the `idx` from the one labeled `ligolo`, which was set up after running `start`
  - Then set up routing with: `route add 192.168.0.0 mask 255.255.255.0 0.0.0.0 if {interface_index}`
